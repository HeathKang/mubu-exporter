...
def get_url(doc_id, doc_name, jwt_token):
    ...
    res = requests.post(url=GET_SINGLE_DOC_PATH, headers=headers, json=payload)
...
def write_to_pdf(data, doc_name, jwt_token):
    ...
    res = requests.post(url=EXPORT_SINGLE_DOC_PATH, headers=headers, json=payload)
...
def write_to_word(data, doc_name, jwt_token):
    headers = {"jwt-token": jwt_token, "origin": "https://mubu.com"}
...
def get_all_file_id_names(jwt_token: str) -> dict:
    ...
    res = requests.post(url=GET_ALL_DOCUMENTS_PATH, headers=headers, json=payload)
...
def get_jwt_token(phone: str, password: str) -> str:
    ...
    res = requests.post(url=PHONE_LOGIN_PATH, json=payload)
...
def main():
    ...
    with console.status("[bold green] Fetching data...") as status:
        for doc_id, doc_name in file_id_names.items():
            ...
            export_to_file(data, doc_name, token)
